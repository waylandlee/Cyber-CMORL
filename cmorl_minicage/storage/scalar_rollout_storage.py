...
class ScalarRolloutStorage:
    ...
    def reset(self) -> None:
        ...
        self.preferences = torch.zeros(
            self.num_steps + 1, self.num_envs, self.preference_dim, device=self.device
        )
        ...
        self.value_preds = torch.zeros(
            self.num_steps + 1, self.num_envs, device=self.device
        )
        self.returns = torch.zeros(self.num_steps + 1, self.num_envs, device=self.device)
    ...
    def insert(
        ...
        self.preferences[self.step + 1].copy_(preference)
    ...
    def feed_forward_generator(self, num_mini_batch: int) -> list[ScalarRolloutBatch]:
        ...
        preferences = self.preferences[:-1].reshape(batch_size, self.preference_dim)
        ...
        batches: list[ScalarRolloutBatch] = []
        for start in range(0, batch_size, mini_batch_size):
            ...
            batches.append(
                ScalarRolloutBatch(
                    obs=obs[indices],
                    preference=preferences[indices],
