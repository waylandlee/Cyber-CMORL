...
def _sample_env_preferences(config, rng: np.random.Generator, num_envs: int, obj_dim: int) -> np.ndarray:
...
def _collect_rollout(env, model, storage, device, config, rng) -> torch.Tensor:
    ...
    preference_tensor = torch.as_tensor(preference_batch, dtype=torch.float32, device=device)
    ...
    for _ in range(storage.num_steps):
        ...
        reward_tensor = torch.as_tensor(scalar_reward, dtype=torch.float32, device=device)
...
def _ppo_update(model, optimizer, storage, config) -> dict[str, float]:
    ...
    for _ in range(config.ppo_epochs):
        for batch in storage.feed_forward_generator(config.num_mini_batch):
            batch_advantages = (batch.advantages - advantages.mean()) / (advantages.std() + 1e-8)
            values, log_probs, entropy = model.evaluate_actions(
                batch.obs, batch.preference, batch.actions
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train a preference-conditioned PPO policy on MiniCAGE."
    )
