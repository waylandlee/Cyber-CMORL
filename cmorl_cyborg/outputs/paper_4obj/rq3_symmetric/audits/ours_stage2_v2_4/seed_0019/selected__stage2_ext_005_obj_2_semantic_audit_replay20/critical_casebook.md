...
- Basic info: `episode_id=episode_012` `env_idx=2` `env_seed=2031` `risk_tier=Tier 1 Near-Miss`
- Returns: `security=-170.6000` `business=-28.1250` `cost=-20.0900`
...
- step 2: Blue `Monitor` -> `-`; Red `ExploitRemoteService` -> `-`; new=User4; recovered=-; critical_after=-
- step 4: Blue `Monitor` -> `-`; Red `DiscoverNetworkServices` -> `-`; new=-; recovered=-; critical_after=-
- step 5: Blue `Monitor` -> `-`; Red `ExploitRemoteService` -> `-`; new=Enterprise0; recovered=-; critical_after=-
- step 6: Blue `Restore` -> `Enterprise0`; Red `PrivilegeEscalate` -> `Enterprise0`; new=-; recovered=Enterprise0; critical_after=-
- step 7: Blue `Monitor` -> `-`; Red `ExploitRemoteService` -> `-`; new=Enterprise0; recovered=-; critical_after=-
- step 99: Blue `Monitor` -> `-`; Red `ExploitRemoteService` -> `-`; new=Enterprise0; recovered=-; critical_after=-
...
- `Restore -> Enterprise0`: `3901` containment steps
- `Restore -> Enterprise1`: `3619` containment steps
