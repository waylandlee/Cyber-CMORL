...
- Basic info: `episode_id=episode_006` `env_idx=0` `env_seed=25` `risk_tier=Tier 3 Persistent Critical Breach`
- Returns: `security=-1496.0500` `business=-170.3150` `cost=-15.5400`
...
- step 5: Blue `Analyse` -> `User2`; Red `ExploitRemoteService` -> `-`; new=Enterprise1; recovered=-; critical_after=-
- step 6: Blue `Remove` -> `User4`; Red `PrivilegeEscalate` -> `Enterprise1`; new=-; recovered=-; critical_after=-
- step 7: Blue `DecoyVsftpd` -> `Op_Server0`; Red `DiscoverRemoteSystems` -> `Enterprise`; new=-; recovered=-; critical_after=-
- step 8: Blue `DecoyVsftpd` -> `Enterprise0`; Red `DiscoverNetworkServices` -> `-`; new=-; recovered=-; critical_after=-
...
- step 15: Blue `Analyse` -> `User2`; Red `Impact` -> `Op_Server0`; new=-; recovered=-; critical_after=Op_Server0
- step 99: Blue `DecoyTomcat` -> `User0`; Red `Impact` -> `Op_Server0`; new=-; recovered=-; critical_after=Op_Server0
...
- Basic info: `episode_id=episode_016` `env_idx=5` `env_seed=5035` `risk_tier=Tier 3 Persistent Critical Breach`
- Returns: `security=-1498.0500` `business=-169.1525` `cost=-15.9400`
- Key times: `enterprise_foothold_step=5` `first_critical_hit_step=12` `first_relevant_blue_response=7`
...
- step 6: Blue `Analyse` -> `User1`; Red `PrivilegeEscalate` -> `Enterprise1`; new=-; recovered=-; critical_after=-
...
- step 14: Blue `DecoyApache` -> `User1`; Red `Impact` -> `Op_Server0`; new=-; recovered=-; critical_after=Op_Server0
- step 15: Blue `Analyse` -> `User2`; Red `Impact` -> `Op_Server0`; new=-; recovered=-; critical_after=Op_Server0
