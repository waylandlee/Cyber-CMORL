...
- Basic info: `episode_id=episode_004` `env_idx=1` `env_seed=1011` `risk_tier=Tier 1 Near-Miss`
- Returns: `security=-171.6000` `business=-28.4850` `cost=-23.3600`
...
- step 2: Blue `Analyse` -> `User3`; Red `ExploitRemoteService` -> `-`; new=User2; recovered=-; critical_after=-
- step 4: Blue `Analyse` -> `User3`; Red `DiscoverNetworkServices` -> `-`; new=-; recovered=-; critical_after=-
- step 5: Blue `Analyse` -> `User3`; Red `ExploitRemoteService` -> `-`; new=Enterprise1; recovered=-; critical_after=-
- step 6: Blue `Restore` -> `Enterprise1`; Red `PrivilegeEscalate` -> `Enterprise1`; new=-; recovered=Enterprise1; critical_after=-
- step 7: Blue `Analyse` -> `Enterprise0`; Red `ExploitRemoteService` -> `-`; new=Enterprise1; recovered=-; critical_after=-
- step 99: Blue `Analyse` -> `User3`; Red `ExploitRemoteService` -> `-`; new=Enterprise1; recovered=-; critical_after=-
...
- Returns: `security=-171.6000` `business=-28.3050` `cost=-23.3600`
- Key times: `enterprise_foothold_step=5` `first_critical_hit_step=None` `first_relevant_blue_response=6`

Key sequence:
- step 0: Blue `Analyse` -> `User3`; Red `DiscoverRemoteSystems` -> `User`; new=-; recovered=-; critical_after=-
- step 1: Blue `Analyse` -> `User3`; Red `DiscoverNetworkServices` -> `-`; new=-; recovered=-; critical_after=-
- step 2: Blue `Analyse` -> `User3`; Red `ExploitRemoteService` -> `-`; new=User3; recovered=-; critical_after=-
- step 4: Blue `Analyse` -> `User3`; Red `DiscoverNetworkServices` -> `-`; new=-; recovered=-; critical_after=-
...
- step 7: Blue `Analyse` -> `User3`; Red `ExploitRemoteService` -> `-`; new=Enterprise0; recovered=-; critical_after=-
- step 99: Blue `Analyse` -> `User3`; Red `ExploitRemoteService` -> `-`; new=Enterprise0; recovered=-; critical_after=-
...
- Returns: `security=-170.6000` `business=-28.0950` `cost=-23.1600`
- Key times: `enterprise_foothold_step=5` `first_critical_hit_step=None` `first_relevant_blue_response=6`
...
- `Restore -> Enterprise1`: `2397` containment steps
- `Restore -> Enterprise0`: `1363` containment steps
