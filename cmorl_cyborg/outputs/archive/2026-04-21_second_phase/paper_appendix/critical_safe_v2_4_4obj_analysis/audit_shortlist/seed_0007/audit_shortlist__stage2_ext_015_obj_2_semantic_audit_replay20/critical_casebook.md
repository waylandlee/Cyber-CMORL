...
- Basic info: `episode_id=episode_001` `env_idx=5` `env_seed=5008` `risk_tier=Tier 1 Near-Miss`
- Returns: `security=-170.6000` `business=-29.8325` `cost=-24.3500`
- Key times: `enterprise_foothold_step=5` `first_critical_hit_step=None` `first_relevant_blue_response=6`

Key sequence:
- step 0: Blue `Analyse` -> `Enterprise0`; Red `DiscoverRemoteSystems` -> `User`; new=-; recovered=-; critical_after=-
- step 1: Blue `Analyse` -> `Enterprise0`; Red `DiscoverNetworkServices` -> `-`; new=-; recovered=-; critical_after=-
- step 2: Blue `Analyse` -> `Enterprise0`; Red `ExploitRemoteService` -> `-`; new=User1; recovered=-; critical_after=-
...
- step 7: Blue `Analyse` -> `User0`; Red `ExploitRemoteService` -> `-`; new=Enterprise1; recovered=-; critical_after=-
- step 99: Blue `DecoyTomcat` -> `User2`; Red `ExploitRemoteService` -> `-`; new=Enterprise1; recovered=-; critical_after=-
...
- Basic info: `episode_id=episode_006` `env_idx=0` `env_seed=13` `risk_tier=Tier 1 Near-Miss`
- Returns: `security=-172.6000` `business=-29.3825` `cost=-23.6600`
- Key times: `enterprise_foothold_step=5` `first_critical_hit_step=None` `first_relevant_blue_response=6`

Key sequence:
- step 0: Blue `Analyse` -> `Enterprise0`; Red `DiscoverRemoteSystems` -> `User`; new=-; recovered=-; critical_after=-
- step 1: Blue `Analyse` -> `Enterprise0`; Red `DiscoverNetworkServices` -> `-`; new=-; recovered=-; critical_after=-
- step 2: Blue `DecoySmss` -> `Enterprise2`; Red `ExploitRemoteService` -> `-`; new=User3; recovered=-; critical_after=-
- step 4: Blue `Analyse` -> `Enterprise0`; Red `DiscoverNetworkServices` -> `-`; new=-; recovered=-; critical_after=-
- step 5: Blue `Analyse` -> `User3`; Red `ExploitRemoteService` -> `-`; new=Enterprise0; recovered=-; critical_after=-
- step 6: Blue `Restore` -> `Enterprise0`; Red `PrivilegeEscalate` -> `Enterprise0`; new=-; recovered=Enterprise0; critical_after=-
- step 7: Blue `Analyse` -> `Enterprise0`; Red `ExploitRemoteService` -> `-`; new=Enterprise0; recovered=-; critical_after=-
- step 99: Blue `Analyse` -> `Enterprise1`; Red `ExploitRemoteService` -> `-`; new=Enterprise0; recovered=-; critical_after=-
...
- Returns: `security=-170.6000` `business=-29.0975` `cost=-23.2600`
- Key times: `enterprise_foothold_step=5` `first_critical_hit_step=None` `first_relevant_blue_response=6`

Key sequence:
- step 0: Blue `Analyse` -> `Enterprise0`; Red `DiscoverRemoteSystems` -> `User`; new=-; recovered=-; critical_after=-
- step 1: Blue `Analyse` -> `Enterprise0`; Red `DiscoverNetworkServices` -> `-`; new=-; recovered=-; critical_after=-
- step 2: Blue `Analyse` -> `Enterprise0`; Red `ExploitRemoteService` -> `-`; new=User3; recovered=-; critical_after=-
- step 4: Blue `Sleep` -> `-`; Red `DiscoverNetworkServices` -> `-`; new=-; recovered=-; critical_after=-
...
- `restore`: `3756` pre-critical steps

### Pre-critical compromised-target recovery counts

- `Restore -> Enterprise1`: `2068` containment steps
- `Restore -> Enterprise0`: `1688` containment steps
