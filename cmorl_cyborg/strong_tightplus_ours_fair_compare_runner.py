...
DEFAULT_PILOT_SEEDS = (19, 7)
DEFAULT_FULL_SEEDS = (7, 11, 19)
DEFAULT_CONSTRAINT_TOLERANCE = -0.5
DEFAULT_CONSTRAINED_UPDATES = 8
DEFAULT_BARRIER_COEF = 35.0
DEFAULT_BETA_MIN = 1.002
DEFAULT_BETA_MAX = 1.010
DEFAULT_SELECTED_EVAL_EPISODES = 40
DEFAULT_REEVALUATED_EVAL_EPISODES = 3

BASE_METHOD_NAME = "ours_stage2_fair"
METHOD_NAME = "ours_stage2_fair_tightplus"
DISPLAY_NAME = "Ours Stage2 Tight+"
RUNNER_DIRNAME = "fair_compare_tightplus_runner"
COMPARE_PLOT_NAME = "fair_compare_table_b_tight_with_tightplus_ours.png"
SUMMARY_CSV_NAME = "reevaluated_tight_feasible_set_summary_with_tightplus_ours.csv"
SUMMARY_JSON_NAME = "reevaluated_tight_feasible_set_summary_with_tightplus_ours.json"
SUMMARY_FIGURE_NAME = "reevaluated_tight_feasible_set_quality_with_tightplus_ours.png"
...
def _run_module(module: str, args: list[str], *, progress: ProgressTracker, label: str) -> None:
...
def _materialize_stage2_config(
    *,
    ...
    payload["ipo"] = ipo
    config_path = _generated_config_root() / f"stage2_fair_tightplus_seed_{seed:04d}.yaml"
...
def _run_training_for_seed(
    *,
    ...
    label = f"train tightplus_ours seed_{seed:04d}"
...
def _reevaluate_seed(
    *,
    ...
    reevaluate_mod.COLORS[METHOD_NAME] = "#e45756"
...
def _aggregate_selected_policy(*, metrics_paths: list[Path], progress: ProgressTracker) -> Path:
    label = "aggregate tightplus selected-policy tight metrics"
    step_start = progress.start_step(label)
...
def _plot_selected_policy_compare(*, new_aggregate_path: Path, progress: ProgressTracker) -> Path:
    label = "plot tightplus selected-policy compare"
    step_start = progress.start_step(label)
    output_path = _aggregated_root() / COMPARE_PLOT_NAME
    baseline_paths = [
        _aggregated_root() / "ours_stage2_fair_tight.json",
        _aggregated_root() / "ours_stage2_fair_tighter_tight.json",
        new_aggregate_path,
...
def _aggregate_reevaluated_compare(
    *,
    ...
    reevaluate_mod.DISPLAY_NAMES[METHOD_NAME] = DISPLAY_NAME
    reevaluate_mod.COLORS[METHOD_NAME] = "#e45756"
    label = "aggregate tightplus reevaluated set metrics"
    step_start = progress.start_step(label)
    new_row = reevaluate_mod._aggregate_method_rows(METHOD_NAME, seed_rows)

    baseline_payload = load_json(
        _aggregated_root() / "reevaluated_tight_feasible_set_summary_with_tighter_ours.json"
    )
    ...
    save_json(
        json_path,
        {
            ...
            "baseline_summary_path": str(
                (_aggregated_root() / "reevaluated_tight_feasible_set_summary_with_tighter_ours.json").resolve()
            ),
...
def run_strong_tightplus_ours_fair_compare(
    *,
    ...
    try:
        for seed in pilot_seed_order:
            ...
            eval_input = _copy_eval_input_for_seed(seed=seed, train_buffer_path=train_buffer, progress=progress)
            ...
            manifest["per_seed"][f"seed_{seed:04d}"] = {
                ...
                "reevaluated_feasible_candidate_count": int(seed_summary["reevaluated_feasible_candidate_count"]),
        ...
        pilot_has_feasible = any(int(row["reevaluated_feasible_candidate_count"]) > 0 for row in seed_rows)
        ...
        selected_aggregate_path = _aggregate_selected_policy(metrics_paths=metrics_paths, progress=progress)
        ...
        final_summary = {
            ...
            "pilot_has_reevaluated_feasible_candidate": any(
                int(row["reevaluated_feasible_candidate_count"]) > 0 for row in seed_rows[: len(pilot_seed_order)]
            ),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Stronger tight-aware fair-compare rerun for Ours Stage2. "
            "Uses a more aggressive constrained Stage 2 and runs pilot seeds first."
        )
    )
    ...
    outputs = run_strong_tightplus_ours_fair_compare(
